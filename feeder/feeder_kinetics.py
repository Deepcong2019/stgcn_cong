...
class Feeder_kinetics(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        self.N = len(self.sample_name)  # sample,视频个数
        self.C = 3  # channel,(X,Y,S)代表一个点的信息(位置+预测的可能性)
        self.T = 300  # frame,一个视频的帧数paper规定是300帧，不足的重头循环，多的clip
        self.V = 26  # joint,根据不同的skeleton获得的节点数而定，coco是18个节点
        self.M = self.num_person_out  # person, 人数, paper中将人数限定在最大2个人
